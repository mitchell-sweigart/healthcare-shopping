class NegotiatedRatesController < ApplicationController
    ...
    def index
        ...
        @negotiated_rates = NegotiatedRate.where(["billing_code LIKE :code AND health_plan_id like :health_plan_id", {:code => params[:query], :health_plan_id => health_plan_id}]).where(billing_code_modifier: nil).or(NegotiatedRate.where(["billing_code LIKE :code", {:code => params[:query]}]).where.not(billing_code_modifier: ["['52']","['53']","['73']","['74']","['52', 'PT']","['59']"]))
